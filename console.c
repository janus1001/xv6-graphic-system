...
short vgasnapshot[VGA_0x03_WIDTH * VGA_0x03_HEIGHT];
int cursorpos;
...
void clear(void)
{
    ...
    case 0x12:
        ...
        {
            consolevgaplane(i);
            memset(consolevgabuffer(), 0, 640 * 480 / 8);
        }
        break;
...
void storetextdata() {
    ...
    outb(CRTPORT, 14);
    cursorpos = inb(CRTPORT + 1) << 8;
    outb(CRTPORT, 15);
    cursorpos |= inb(CRTPORT + 1);
    
    memmove(vgasnapshot, VGA_0x03_MEMORY, VGA_0x03_HEIGHT * VGA_0x03_WIDTH * sizeof(short));
}

// Store text in buffer
void restoretextdata() {
    ushort* crt = VGA_0x03_MEMORY;

    outb(CRTPORT, 14);
    outb(CRTPORT + 1, cursorpos >> 8);
    outb(CRTPORT, 15);
    outb(CRTPORT + 1, cursorpos);
    crt[cursorpos] = ' ' | 0x0700;

    memmove(VGA_0x03_MEMORY, vgasnapshot, VGA_0x03_HEIGHT * VGA_0x03_WIDTH * sizeof(short));
}
